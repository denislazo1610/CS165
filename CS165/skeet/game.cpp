...
Bird* Game :: createBird()
{
   ...
   if( randomBird == 1)
   {
       ...
       Point position(-200 ,random(-200, 200));
       newBird -> setPoint(position);

       if((newBird -> getPoint()).getY() > 0)
       {
           Velocity velocidad(random(3, 7), random(-4,1));
           newBird -> setVelocity(velocidad);
       }
       else 
       {
           Velocity velocidad(random(3, 7), random(0, 5));
           newBird -> setVelocity(velocidad);
       }
